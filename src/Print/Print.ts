...
export default class {
  ...
  constructor(option: Options) {
    ...
    this.settings.fId = `printArea_${Date.now()}`;
  }

  print(id: string, option?: Options) {
    const iframe = document.getElementById(this.settings.fId);
    if (iframe) {
      document.body.removeChild(iframe as any);
    }
    this.settings = {
